...
app = Flask(
    __name__, template_folder="./appendix/templates", static_folder="./appendix/static"
)
...
pytvzhen_api_request_counter = metrics.counter(
    ...
    labels={
        "base_url": lambda: url_rule_to_base(request.url_rule),
        "stage": lambda: pytvzhen_stage(),
...
def video_upload():
    ...
    if file and video_extension_allowed(file.filename):
        ...
        return jsonify(
            {
                "message": log_info_return_str(
                    f"Video {file.filename} uploaded as {video_fn}"
                ),
                "video_id": video_id,
            }
        )
        ...
        return jsonify(
            {
                "message",
                log_error_return_str(
                    f"Video upload failed: {file.filename} extension not allowed"
                ),
            }
        )
...
def yt_thumbnail(video_id):
    ...
    try:
        ...
        raise Exception(
            f"thumbnail download failed: {response.status_code} {response.content}"
        )
    ...
    return (
        jsonify(
            {
                "message": log_error_return_str(
                    f"An error occurred while downloading video thumbnail {video_id} to {thumbnail_save_path}: {exception}"
                )
            }
        ),
        500,
...
def yt_download(video_id):
    ...
    if os.path.exists(video_save_path) and os.path.exists(video_fhd_save_path):
        return (
            jsonify(
                {
                    "message": log_info_return_str(
                        f"Video already exists at {video_save_path}, skip downloading."
                    ),
                    "video_id": video_id,
                }
            ),
        ...
        if yt.length > app.config["VIDEO_MAX_DURATION"]:
            return (
                jsonify(
                    {
                        "message": log_error_return_str(
                            f'Video duration is too long. Please select videos with duration less than {app.config["VIDEO_MAX_DURATION"]} seconds. '
                        )
                    }
                ),
                400,
    ...
    return (
        jsonify(
            {
                "message": log_error_return_str(
                    f"An error occurred while downloading video {video_id} to {video_save_path}: {exception}"
                )
            }
        ),
        500,
...
def yt_serve(video_id):
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Video {video_fn} not found at {output_path}"
                )
            }
        ),
        404,
...
def extra_audio(video_id):
    ...
    video_path, audio_path = os.path.join(output_path, video_fn), os.path.join(
        output_path, audio_fn
    )
    ...
    if os.path.exists(audio_path):
        return (
            jsonify(
                {
                    "message": log_info_return_str(
                        f"Audio already exists at {audio_path}, skip extracting."
                    ),
                    "video_id": video_id,
                }
            ),
    ...
    if not os.path.exists(video_path):
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Video to extract {video_fn} not found at {output_path}, please download it first"
                )
            }
        ), 404
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": log_info_return_str(
                        f"Extracted audio {audio_fn} from {video_path} successfully."
                    ),
                    "video_id": video_id,
                }
            ),
    ...
    return (
        jsonify(
            {
                "message": log_error_return_str(
                    f"An error occurred while extracting audio {audio_fn} from {video_fn}: {exception}"
                )
            }
        ),
        500,
...
def audio_serve(video_id):
    ...
    _, audio_path = os.path.join(output_path, video_fn), os.path.join(
        output_path, audio_fn
    )
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Extracted audio {audio_fn}  not found at {audio_path}"
                )
            }
        ),
        404,
...
def remove_audio_bg(video_id):
    ...
    _, audio_path, audio_no_bg_path, audio_bg_fn_path = (
        os.path.join(output_path, video_fn),
        ...
        jsonify(
            {
                "message": log_warning_return_str(
                    ...
                    f"not found at {output_path}, please extract it first"
                )
            }
        ), 404
    ...
    return (
        jsonify(
            {
                "message": log_error_return_str(
                    f"An error occurred while removing background music for {audio_fn} as {audio_no_bg_fn} and {audio_bg_fn_path}: {exception}"
                )
            }
        ),
        500,
...
@pytvzhen_api_request_counter
@app.route("/remove_bg_audio_status", methods=["GET"])
def remove_bg_audio_status():
    try:
        task_id = request.args.get("task_id")
        if not task_id:
            return jsonify({"error": "Missing task_id parameter"}), 400

        resp = requests.get("http://localhost:8199/audio_sep_status?task_id={task_id}")
        return (
            jsonify(resp.json()),
            resp.status_code,
        )
...
def get_audio_transcribe_status():
    try:
        ...
        resp = requests.get(
            "http://localhost:8199/audio_transcribe_status?task_id={task_id}"
        )
        return (
            jsonify(resp.json()),
            resp.status_code,
...
def audio_no_bg_serve(video_id):
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Audio without background music {audio_no_bg_fn} not found at {audio_no_bg_path}"
                )
            }
        ),
        404,
...
def audio_bg_serve(video_id):
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Audio with background music only {audio_bg_fn} not found at {audio_bg_path}"
                )
            }
        ),
        404,
...
def transcribe(video_id):
    ...
    en_srt_fn, en_srt_merged_fn, audio_no_bg_fn = (
        f"{video_id}_en.srt",
        f"{video_id}_en_merged.srt",
        f"{video_id}_no_bg.wav",
    )

    en_srt_path, en_srt_merged_path, audio_no_bg_path = (
        os.path.join(output_path, en_srt_fn),
        ...
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Audio with voice "
                    f"not found at {audio_no_bg_path}, please extract it first"
                )
            }
        ), 404
    ...
    return (
        jsonify(
            {
                "message": log_error_return_str(
                    f"An error occurred while transcribing SRT from {audio_no_bg_fn} as {en_srt_fn} and {en_srt_merged_fn}: {exception}"
                )
            }
        ),
        500,
...
def srt_en_serve(video_id):
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Transcribed English SRT {en_srt_fn} not found at {en_srt_path}"
                )
            }
        ),
        404,
...
def transhlate_to_zh(video_id):
    ...
    if not os.path.exists(en_srt_merged_path):
        return (
            jsonify(
                {
                    "message": log_warning_return_str(
                        f"English SRT {en_srt_merged_fn} not found at {en_srt_merged_path}"
                    )
                }
            ),
            404,
    ...
    if translateVendor not in ["google", "deepl"] and "gpt" not in translateVendor:
        return (
            jsonify(
                {"message": log_warning_return_str("Unsupported translate vendor.")}
            ),
            404,
        )
    ...
    try:
        ...
        ret = translator.translate_srt(
            source_file_name_and_path=en_srt_merged_path,
        ...
        if ret:
            ...
            return (
                jsonify(
                    {
                        "message": log_warning_return_str(
                            f"{translateVendor} translate failed."
                        )
                    }
                ),
                404,
...
def srt_en_merged_serve(video_id):
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Transcribed English SRT {en_srt_merged_fn} not found at {en_srt_merged_path}"
                )
            }
        ),
        404,
...
def srt_zh_merged_serve(video_id):
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Transcribed English SRT {zh_srt_merged_fn} not found at {zh_srt_merged_path}"
                )
            }
        ),
        404,
...
def voice_connect(video_id):
    ...
    if not os.path.exists(voiceDir):
        return (
            jsonify(
                {
                    "message": log_warning_return_str(
                        f"Voice directory {voiceDir} not found at {output_path}"
                    )
                }
            ),
            404,
    ...
    if ret:
        ...
        return (
            jsonify({"message": log_warning_return_str("Voice connect failed.")}),
            404,
        )
...
def voice_connect_log_serve(video_id):
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Voice connect {warning_log_path} not found at {warning_log_path}"
                )
            }
        ),
        404,
...
def voice_connect_serve(video_id):
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Voice connect {voice_connect_fn} not found at {voice_connect_path}"
                )
            }
        ),
        404,
...
def tts(video_id):
    ...
    if not os.path.exists(srt_path):
        return (
            jsonify(
                {
                    "message": log_warning_return_str(
                        f"Chinese SRT {srt_fn} not found at {output_path}"
                    )
                }
            ),
            404,
    ...
    try:
        ...
        return (
            jsonify(
                {"message": log_info_return_str("tts success."), "video_id": video_id}
            ),
            200,
        )
    except Exception as e:
    ...
    if not os.path.exists(tts_dir):
        return (
            jsonify(
                {
                    "message": log_warning_return_str(
                        f"Voice directory {tts_dir} not found at {output_path}"
                    )
                }
            ),
            404,
...
def video_preview(video_id):
    ...
    if (not os.path.exists(voice_connect_path)) or (not os.path.exists(audio_bg_path)):
        return (
            jsonify(
                {
                    "message": log_warning_return_str(
                        f'Chinese Voice {video_id + "_zh.wav"} not found at {output_path}'
                    )
                }
            ),
            404,
    ...
    # 检查视频
    if (not os.path.exists(video_save_path)) and (
        not os.path.exists(video_fhd_save_path)
    ):
        return jsonify({"message": log_warning_return_str("No video found")}), 404
    ...
    if blocking:
        video_preview_task.apply_async(
            args=(video_source_path, voice_connect_path, audio_bg_path, video_out_path)
        ).get()
        return (
            jsonify(
                {
                    "message": log_info_return_str(
                        f"Video preview {video_id} successfully rendered."
                    )
                }
            ),
            200,
    ...
    task = video_preview_task.delay(
        video_source_path, voice_connect_path, audio_bg_path, video_out_path
    )
    ...
    return (
        jsonify(
            {
                "message": log_info_return_str(
                    f"Submitted video preview task {task.id}."
                ),
...
def video_preview_serve(video_id):
    ...
    return (
        jsonify(
            {
                "message": log_warning_return_str(
                    f"Video preview {video_preview_fn} not found at {video_preview_path}"
                )
            }
        ),
        404,
