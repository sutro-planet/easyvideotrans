...
class ResBlock1(torch.nn.Module):
    ...
    def forward(self, x):
        for c1, c2 in zip(self.convs1, self.convs2):
            xt = torch.nn.functional.silu(x)
            xt = c1(xt)
            xt = torch.nn.functional.silu(xt)
            xt = c2(xt)
            x = xt + x
        return x
...
class LinearSpectrogram(nn.Module):
    ...
    def forward(self, y: torch.Tensor) -> torch.Tensor:
        if y.ndim == 3:  # remove second dim if mono audio
            y = y.squeeze(1)
...
class LogMelSpectrogram(nn.Module):
    ...
    def apply_mel_scale(self, x: torch.Tensor) -> torch.Tensor:
        return torch.matmul(x.transpose(-1, -2), self.fb).transpose(-1, -2)

    def forward(
            self, x: torch.Tensor, return_linear: bool = False, sample_rate: int = None
    ) -> torch.Tensor:
        if sample_rate is not None and sample_rate != self.sample_rate:
            x = torchaudio.functional.resample(x, orig_freq=sample_rate, new_freq=self.sample_rate)
...
class DownsampleFiniteScalarQuantize(nn.Module):
    ...
    def forward(self, z) -> FSQResult:
        ...
        if diff > 0:
            result.z = torch.nn.functional.pad(result.z, (left, right))
        elif diff < 0:
