...
from workloads.lib.fish_speech.modules.tokenizer import FishTokenizer, SEMANTIC_TOKENS
...
def precompute_freqs_cis(seq_len: int, n_elem: int, base: int = 10000) -> torch.Tensor:
    """
    Precomputes frequency tensors for complex exponential (cis)
...
class TransformerBlock(nn.Module):
    ...
    def forward(
            self, x: torch.Tensor, freqs_cis: torch.Tensor, mask: torch.Tensor, input_pos: torch.Tensor = None
    ) -> torch.Tensor:
...
class BaseTransformer(nn.Module):
    ...
    def embed(self, inp: torch.Tensor, share_codebook_embeddings=True) -> torch.Tensor:
        embeds = []
    ...
    def forward(
        ...
        if self.config.tie_word_embeddings:
            token_logits = torch.nn.functional.linear(slow_out, self.embeddings.weight)
        else:
...
class DualARTransformer(BaseTransformer):
    ...
    def forward(
        ...
        codebooks = inp[:, 1:-1, 1:]
        codebooks = torch.nn.functional.pad(codebooks, (0, 1), value=0)
    ...
    def forward_generate(
            ...
            x: torch.Tensor,
            input_pos: Optional[torch.Tensor] = None,
            vq_masks: Optional[torch.Tensor] = None,
    ) -> TransformerForwardResult:
